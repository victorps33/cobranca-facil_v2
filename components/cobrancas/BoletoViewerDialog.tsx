...
export function BoletoViewerDialog({
  ...
  const hasBoletoReal = !!cobranca.boletoUrl;
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        {hasBoletoReal ? (
          ...
          <div className="space-y-4">
            <iframe
              src={cobranca.boletoUrl}
              className="w-full h-[500px] border border-gray-200 rounded-lg"
              title="Boleto bancário"
            />

            {/* Linha digitável real */}
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-2">
                Linha digitável
              </p>
              <code className="font-mono text-sm text-gray-700 tracking-wider select-all">
                {linhaDigitavel}
              </code>
            </div>
          </div>
        ) : (
          /* ── Boleto simulado (fallback) ── */
