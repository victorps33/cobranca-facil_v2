...
import type { Channel, MessageQueue } from "@prisma/client";
...
// ---------------------------------------------------------------------------
// dispatchQueueItem – original queue-based dispatch (backward compat)
// ---------------------------------------------------------------------------
...
export async function processPendingQueue(
  ...
  for (const item of items) {
    ...
    const result = await dispatchQueueItem(item);
    if (result.success) {
      sent++;
    } else {
      failed++;
